...
export default function HomeView({ lang = "tr" }: { lang?: string }) {
    ...
    return (
        <main className="flex flex-col bg-white">
            ...
            {/* --- SMS BAĞIŞ POPUP (MOBİL UYUMLU, ÇİFT KAPATMA & ALT AKSİYON BUTONLU) --- */}
            {showPopup && (
            <>
                <style jsx>{`
                ...
                className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 overflow-y-auto"
                >
                <div
                    onClick={(e) => e.stopPropagation()}
                    className="relative w-full max-w-[420px] sm:max-w-[400px] my-auto flex flex-col"
                    style={{ animation: "popupScale .3s ease-out" }}
                >
                    {/* 1. SAĞ ÜST KAPATMA TUŞU */}
                    <button
                    onClick={() => setShowPopup(false)}
                    aria-label="Popup kapat"
                    className="absolute -top-3.5 -right-3.5 z-50 bg-red-600 text-white w-40 h-40 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow-2xl hover:bg-red-700 hover:scale-110 transition-all duration-200 border-2 border-white cursor-pointer"
                    >
                    <X size={60} strokeWidth={6.5} />
                    </button>

                    {/* SMS BAĞIŞ GÖRSELİ */}
                    <a
                    href="sms:9790?body=BAĞIŞ"
                    className="block overflow-hidden rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-2 border-white/20 bg-white"
                    >
                    <Image
                    ...
                    {/* 2. ALT AKSİYON BUTONLARI */}
                    <div className="mt-3.5 flex flex-col sm:flex-row gap-2.5 w-full">
                    <a
                        href="sms:9790?body=BAĞIŞ"
                        className="flex-1 bg-[#0b5331] hover:bg-[#084326] text-white py-3 px-4 rounded-xl font-bold text-center text-sm sm:text-base shadow-lg transition-all duration-200 flex items-center justify-center gap-2"
                    >
                        <span>SMS ile Bağış Yap</span>
                    </a>

                    <button
                        onClick={() => setShowPopup(false)}
                        className="bg-white/90 hover:bg-white text-gray-800 py-3 px-4 rounded-xl font-semibold text-center text-sm sm:text-base shadow-md transition-all duration-200 border border-gray-200"
                    >
                        Siteye Devam Et
                    </button>
                    </div>
